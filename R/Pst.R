Pst <-
function(Mat,csh=1,col=1,a=1,boot=1000,Pw=0,Rp=0,Ri=0,pe=0.95){
Prep<-function(M){
...
v=Bootk(M,csh,boot,k);v=sort(v);
return(c(v[floor(boot*(1-per)/2+1)],v[ceiling(boot*(1+per)/2)]));}
A=Prep(Mat);
A=MaReIP(A,Ri,Rp);A=MaPw(A,Pw);
if (a==1) {print("Populations sizes are:"); print(Pop(A)); if (col[1]==1) {Q=dim(A)[2]; for (i in 2:Q) {print(names(A)[i]); print(Pstc(A,csh)[i-1]);}}
else {n=length(col); for (i in 1:n) {print(names(A)[col[i]]); print(Pstc(A,csh)[col[i]-1]);}};};
if (a==2) {print("Populations sizes are:"); print(Pop(A)); if (col[1]==1) {Q=dim(A)[2]; for (i in 2:Q) {print(names(A)[i]); print(Pstc(A,csh)[i-1]); print(ConfinterPstk(A,csh=csh,boot=boot,k=i,per=pe));}}
else {n=length(col); for (i in 1:n) {print(names(A)[col[i]]); print(Pstc(A,csh)[col[i]-1]); print(ConfinterPstk(A,csh=csh,boot=boot,k=col[i],per=pe));}}}}
